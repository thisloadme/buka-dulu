...
func (r *CustomerRepository) Create(c *domain.CustomerSegment) error {
	_, err := r.db.Exec(
		`INSERT INTO customer_segments (id, venture_id, name, age_range, buy_context, budget_range, location, consumption_moment, is_too_general, is_locked, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		c.ID, c.VentureID, c.Name, c.AgeRange, c.BuyContext, c.BudgetRange, c.Location, c.ConsumptionMoment, boolToInt(c.IsTooGeneral), boolToInt(c.IsLocked), c.CreatedAt, c.UpdatedAt,
	)
	return err
}
...
func (r *CustomerRepository) FindByVenture(ventureID string) (*domain.CustomerSegment, error) {
	...
	err := r.db.QueryRow(
		`SELECT id, venture_id, name, age_range, buy_context, budget_range, location, consumption_moment, is_too_general, is_locked, created_at, updated_at
		 FROM customer_segments WHERE venture_id = ? ORDER BY created_at DESC LIMIT 1`, ventureID,
...
func (r *CustomerRepository) Update(c *domain.CustomerSegment) error {
	_, err := r.db.Exec(
		`UPDATE customer_segments SET name=?, age_range=?, buy_context=?, budget_range=?, location=?, consumption_moment=?, is_too_general=?, is_locked=?, updated_at=? WHERE id=?`,
		c.Name, c.AgeRange, c.BuyContext, c.BudgetRange, c.Location, c.ConsumptionMoment, boolToInt(c.IsTooGeneral), boolToInt(c.IsLocked), c.UpdatedAt, c.ID,
	)
	return err
}

func (r *CustomerRepository) Lock(id string) error {
	_, err := r.db.Exec(`UPDATE customer_segments SET is_locked=1, updated_at=datetime('now') WHERE id=?`, id)
	return err
}
