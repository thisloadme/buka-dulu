...
func (r *IdeaRepository) FindByVenture(ventureID string) (*domain.Idea, error) {
	idea := &domain.Idea{}
	var aiRawInput, aiRawOutput, refineHistory sql.NullString
	err := r.db.QueryRow(
		`SELECT id, venture_id, raw_input, one_line_concept, target_customer, value_proposition,
		        key_assumptions, early_risks, version, is_locked, status, ai_raw_input, ai_raw_output,
		        refine_history, created_at, updated_at
		...
		&idea.Version, &idea.IsLocked, &idea.Status, &aiRawInput, &aiRawOutput, &refineHistory,
		&idea.CreatedAt, &idea.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, domain.ErrNotFound
	}
	if aiRawInput.Valid {
		idea.AiRawInput = &aiRawInput.String
	}
	if aiRawOutput.Valid {
		idea.AiRawOutput = &aiRawOutput.String
	}
	if refineHistory.Valid && refineHistory.String != "" {
		idea.RefineHistory = &refineHistory.String
	}
	return idea, err
}
...
func (r *IdeaRepository) Update(idea *domain.Idea) error {
	_, err := r.db.Exec(
		...
		idea.KeyAssumptions, idea.EarlyRisks, idea.IsLocked, idea.Status, idea.AiRawOutput,
		idea.RefineHistory, idea.ID,
	)
	return err
}
